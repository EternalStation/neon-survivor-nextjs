...
| 2 | Overheal becomes Shield (3s Static) |
...
```
Applied via `applyHealToPlayer` in `CombatUtils.ts`. Restores `damageAmount × 0.03` HP to the player (owner). **Shockwaves do NOT trigger lifesteal unless Blood-Forged Capacitor is active.**

**L2 — Overheal Shield (Static 3s):**
- If ANY health the player receives would exceed MaxHP, the overflow is stored as a shield chunk.
- Conversion is **1:1** — no efficiency multiplier.
- Duration: **3 seconds, static** (not affected by any multiplier or upgrade).
...
- Spawn location: Random distant position outside player view (following normal spawn logic).
- Death property: Zombies do not drop loot or souls and cannot respawn if they die.
- Spawn delay: 5 seconds after the kill.

## Affected Files

- `CombatUtils.ts` — `applyHealToPlayer` handles overheal shield push.
- `ProjectilePlayerLogic.ts` — Lifesteal heal trigger and Lvl 3 bonus damage.
