...
## Overview
To prevent accidental clicks and enhance the "tactical hacker" aesthetic of Neon Survivor, a 0.2-second security protocol is active whenever the Upgrade Menu appears. During this window, the player cannot select an upgrade, and a "decryption" animation is displayed.

## Animation Mechanics

### 1. Initial State (Locked)
- **Duration**: 200ms (500ms for Special/Void Tech).
- **Visuals**:
...
- After the protocol duration (200ms or 500ms), the `canSelect` state becomes `true`.
