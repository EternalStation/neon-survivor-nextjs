...
export function updateElitePentagon({ enemy, state, dist, dx, dy, currentSpd, pushX, pushY }: EnemyUpdateContext) {
    ...
    if (age > 60) {
        if ((enemy.minionCount || 0) > 0) {
            if (!enemy.lastAttack) enemy.lastAttack = state.gameTime;
    ...
    if (enemy.summonState === 1) {
        if (state.gameTime > (enemy.timer || 0)) {
            spawnMinion(state, enemy, true, 3);
            enemy.lastAttack = state.gameTime;
            enemy.summonState = 0;
