...
export const getIcon = (type: string, color: string) => {
    ...
    switch (type) {
        case 'dmg': // Flat Dmg: White Hex + Cyan Dot (Burst)
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: filterGlow }}>
                    ...
                    <line x1="50" y1="50" x2="50" y2="25" stroke={color} strokeWidth="2" opacity="0.6" />
                </svg>
            );

        case 'dmg_m': // Dmg Multi: Hex + X + Orbiting Dots
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: `drop-shadow(0 0 4px ${color})` }}>
                    <path d="M50 10 L85 30 L85 70 L50 90 L15 70 L15 30 Z"
                        fill="none" stroke="white" strokeWidth="3" />
                    <text x="50" y="62" textAnchor="middle" fill={color} fontSize="40" fontFamily="Orbitron, sans-serif" fontWeight="900">%</text>
            ...
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: `drop-shadow(0 0 4px ${color})` }}>
                    <path d="M60 10 L30 50 L55 50 L40 90 L70 50 L45 50 Z"
                        ...
                        <animate attributeName="fill" values={`${color};#E0FFFF;${color}`} dur="0.5s" repeatCount="indefinite" />
                    </path>
                </svg>
            );
        ...
        case 'hp_f': // Flat Max Health: Red Outline Heart (Wider) + Growing Animation
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: `drop-shadow(0 0 3px ${color})` }}>
                    <path d={WIDE_HEART_PATH} fill="none" stroke={color} strokeWidth="4" style={{ transformOrigin: '50% 50%' }}>
        ...
        case 'hp_m': // Health Multiplier: Red Outline Heart + X + Growing Animation
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: `drop-shadow(0 0 3px ${color})` }}>
                    ...
                    <text x="50" y="65" textAnchor="middle" fill={color} fontSize="36" fontFamily="Orbitron, sans-serif" fontWeight="900">%</text>
                </svg>
            );
        ...
        case 'arm_f': // Armor: Shield
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: `drop-shadow(0 0 2px ${color})` }}>
                    <path d="M20 20 L50 10 L80 20 L80 50 C 80 75, 50 90, 50 90 C 50 90, 20 75, 20 50 Z"
                        fill="none" stroke={color} strokeWidth="4" />
                    <path d="M35 35 L50 25 L65 35 L65 50 C 65 65, 50 75, 50 75 C 50 75, 35 65, 35 50 Z"
                        fill={color} opacity="0.5" />
                </svg>
            );

        case 'arm_m': // Armor Multiplier: Shield + %
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: `drop-shadow(0 0 2px ${color})` }}>
                    <path d="M20 20 L50 10 L80 20 L80 50 C 80 75, 50 90, 50 90 C 50 90, 20 75, 20 50 Z"
                        fill="none" stroke={color} strokeWidth="4" />
                    <text x="50" y="58" textAnchor="middle" fill={color} fontSize="40" fontFamily="Orbitron, sans-serif" fontWeight="900">%</text>
                </svg>
            );

        case 'xp_f': // XP Bonus: Progress Bar
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: `drop-shadow(0 0 4px ${color})` }}>
                    <rect x="20" y="60" width="60" height="8" fill="none" stroke={color} strokeWidth="1" rx="2" />
                    <rect x="22" y="62" width="56" height="4" fill={color}>
                        <animate attributeName="width" values="0;56;0" dur="2s" repeatCount="indefinite" />
                    </rect>
                </svg>
            );

        case 'xp_m': // XP Multi: Progress Bar + %
            return (
                <svg viewBox="0 0 100 100" width="100%" height="100%" style={{ filter: `drop-shadow(0 0 4px ${color})` }}>
                    <rect x="20" y="60" width="60" height="8" fill="none" stroke={color} strokeWidth="1" rx="2" />
                    <rect x="22" y="62" width="56" height="4" fill={color}>
                    ...
                    <text x="50" y="50" textAnchor="middle" fill={color} fontSize="32" fontFamily="Orbitron, sans-serif" fontWeight="900">%</text>
                </svg>
            );

        default:
            return <text x="50" y="50" textAnchor="middle" fill={color}>?</text>;
    }
};
