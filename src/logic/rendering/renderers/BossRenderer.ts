...
export function renderAnomalyStats(ctx: CanvasRenderingContext2D, e: Enemy, state: GameState) {
    if (!e.isAnomaly || e.dead) return;
    ...
    if (distToPlayer < burnRadius + 400) {
        ctx.save();

        const boxX = e.x;
        const boxY = e.y - e.size * 2 - 60;
        ...
        const wallDmg = Math.round(e.maxHp * 0.1);
        const burnDmgPct = (0.05 + (gen * 0.01) + (e.bonusBurnPct || 0)) * 100;
        ...
        ctx.fillText(`${formatLargeNumber(Math.round(e.hp))} / ${formatLargeNumber(Math.round(e.maxHp))}`, rectW / 2 - padding, statsY);
        ...
        ctx.fillText(`${burnDmgPct.toFixed(1)}% HP / SEC`, rectW / 2 - padding, statsY);

        statsY += lineH;
        ctx.textAlign = 'left';
        ctx.fillStyle = '#cbd5e1';
        ctx.fillText(`WALL:`, statsX, statsY);
        ctx.textAlign = 'right';
        ctx.fillStyle = '#ef4444';
        ctx.fillText(`${formatLargeNumber(wallDmg)} IMPACT`, rectW / 2 - padding, statsY);
